...
function p5test(days,doses,spacingtype, correctionflag)
% PK and PD models on time domain
		...
		if(spacingtype ==1)	
			for k = 1:(days-1)
				if (k==varnext)
					d(k)=a;
					varbuffer2=varnext;
					varnext=varnext+varbuffer;
					varbuffer=varbuffer2;
					if (k ==1)
						varnext=varnext+1
					end
	...
	% Euler's aproximation method for diferential functions
		...
		for k = 2:(days-1)
			if (spacingtype==2)
				randomspac(k) = randi([0 1],1); %use bernoulli test to see if a dose is taken
				randomspac(k) = randomfactor * a ;
				if boolean(correctionflag) % when using the correction model,
								%see if a dose the last day. If not take a double dose.
									%Assuming upscale of 2 as ideal
					if rem(k-1,2)==0 && randomspac(k-2)
						randomspac(k) = 2 * a;
					end
				end
				randomspac(k) = randomfactor
			end
			t(k+1) = k*h;
        		if boolean(truerandomflag)
				c = m * [c1(k); c2(k)] + s * delta * randomspac(k);
			else
